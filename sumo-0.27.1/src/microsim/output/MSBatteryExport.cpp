...
// method definitions
// ===========================================================================
void MSBatteryExport::write(OutputDevice& of, SUMOTime timestep,
		int precision) {
	of.openTag(SUMO_TAG_TIMESTEP).writeAttr(SUMO_ATTR_TIME,
	...
	for (; it != end; ++it) {
		...
		Position pos = veh->getLane()->getShape().positionAtOffset(
				veh->getPositionOnLane());

		if (static_cast<MSDevice_Battery*>(veh->getDevice(
				typeid(MSDevice_Battery))) != 0) {

			MSDevice_Battery* batteryToExport =
					dynamic_cast<MSDevice_Battery*>(veh->getDevice(
							typeid(MSDevice_Battery)));
			...
			if (batteryToExport->getMaximumBatteryCapacity() > 0) {
				...
				// Write ActBatKap
				of.writeAttr("actualBatteryCapacity",
						batteryToExport->getActualBatteryCapacity());

				// Write MaxBatKap
				of.writeAttr("maximumBatteryCapacity",
						batteryToExport->getMaximumBatteryCapacity());

				// Write Charging Station ID
				of.writeAttr("chargingStationId",
						batteryToExport->getChargingStationID());

				// Write Charge charged in the Battery
				of.writeAttr("energyCharged",
						batteryToExport->getEnergyCharged());

				// Write ChargeInTransit
				if (batteryToExport->isChargingInTransit()) {
					of.writeAttr("energyChargedInTransit",
							batteryToExport->getEnergyCharged());
				} else {
				...
				if (batteryToExport->isChargingStopped()) {
					of.writeAttr("energyChargedStopped",
							batteryToExport->getEnergyCharged());
				} else {
				...
				of.writeAttr("timeStopped",
						batteryToExport->getVehicleStopped());


				//FOR UPDATE ACTUAL BATERRY CHARGE STATUS
				std::string name = "actualBatteryCapacity";
				std::ostringstream strs;
				strs << batteryToExport->getActualBatteryCapacity();
				std::string value = strs.str();
				((SUMOVehicleParameter&) veh->getParameter()).addParameter(name,
						value);
